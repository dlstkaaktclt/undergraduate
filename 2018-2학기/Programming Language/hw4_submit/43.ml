...
let rec getcond (i : id) (req : require list) : cond list = match req with
	| [] -> raise (Error "Error : getcond SomeThing wrong")
...
let rec getwannabe (i : id) (con : cond) (before : (id * giftset) list) : giftset = 
	...
	| Common (acon,bcon) -> (setIntersect (getwannabe i acon before) (getwannabe i bcon before)) 				(* a랑 b 컨디션 고려해서, 두 결과로 만들어진 리스트를 foldleft로 중복없이 이쁘게 합치자 *)
	| Except (acon,gl) -> (setMinus (getwannabe i acon before) (listToset gl))                      (* a condition으로 따온 리스트에서 gl에 해당하는 리스트의 멤버들을 없애버리자. filter를 사용하면 될듯. *)
...
let findshop (req : require list) (before : (id * giftset) list) : (id * giftset) list = (* 한번 돌리기 *)
	...
	(A,agift)::(B,bgift)::(C,cgift)::(D,dgift)::(E,egift)
	(*
	(addgiftlist before ((A,Agift)::(B,Bgift)::(C,Cgift)::(D,Dgift)::(E,Egift)))
	*)
let findshoplist (req : require list) (before : (id * giftset) list) : (id * giftset) list = 
...
let shoppingList (req : require list) : (id * gift list) list = 
	(findshoplist (sotrReq req)  [(A,[])::(B,[])::(C,[])::(D,[])::(E,[])])
