  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
	 ...
	 (*
	  let compareMem (a : loc) (b : loc) : int = 
	  if ((fst a) < (fst b)) then -1
	  else (if ((fst a) > (fst b)) then 1 else 0) in
	  *)
	  let rec find_locataion_reachable_location (l : loc) :loc list =
	  let memval = (List.assoc l m) in
	  match memval with
	  | R r -> l::(List.concat (List.map find_locataion_reachable_location (snd (List.split r))))
	  | L valoc -> (l::find_locataion_reachable_location valoc)
	  | _ -> [l] in
	  let envfindreachablefunc (reachablelst : loc list) ((str,evalue) : (string * evalue)) : loc list = 
		match evalue with
		| Loc l -> (find_locataion_reachable_location l) @ reachablelst
		| Proc p -> reachablelst in

	  let contfindreachablefunc (reachablelst : loc list) (continue : (command * environment)) : loc list =
	  (List.fold_left envfindreachablefunc [] (snd continue)) @ reachablelst in
	  
	  let stkfindreachablefunc (reachablelst : loc list) (sval : svalue) : loc list =
	    match sval with 
		| V v -> (match v with
				  | R r -> (List.concat (List.map find_locataion_reachable_location (snd (List.split r)))) @ reachablelst
				  | L l -> (find_locataion_reachable_location l) @ reachablelst
				  | _ -> reachablelst
				  )
		| M env -> (envfindreachablefunc (reachablelst) env) @ reachablelst
		| _ -> reachablelst in
	
	  	
      let _ = reachable_locs := [] in
	  
	  let envreachable : loc list = (List.fold_left envfindreachablefunc [] e) in
	  let stackreachable : loc list = (List.fold_left stkfindreachablefunc [] s) in
	  let contreachable : loc list = (List.fold_left contfindreachablefunc [] k) in
	  let reachablebase : int list = (fst (List.split (envreachable @ stackreachable @ contreachable))) in
	  let memreachable : loc list = (fst (List.split (List.filter (fun ((b,o),_) -> List.mem b reachablebase) m))) in 
	  
	  (*let sorted_locs = (List.sort_uniq compareMem reachable) in *)
	  let _ = reachable_locs := memreachable in
