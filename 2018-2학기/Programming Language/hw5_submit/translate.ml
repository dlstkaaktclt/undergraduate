...
module Translator = struct
  ...
  let rec trans : K.program -> Sm5.command = function
	...
	| K.WHILE (condexp, bodyexp) ->
		let pushWhile = [Sm5.PUSH (Sm5.Fn ("",[Sm5.BIND "whileloop"] @ (trans condexp) @ [Sm5.JTR ((trans bodyexp) @ [Sm5.POP] @ (trans (K.CALLV ("whileloop",K.UNIT))), [(Sm5.PUSH (Sm5.Val (Sm5.Unit)))])]))] in
		pushWhile
		@ [Sm5.BIND "whileloop"] 
	...
	| K.FOR (id,initexp,endexp,bodyexp) ->
		...
		let pushFor = [Sm5.PUSH (Sm5.Fn ("",[Sm5.BIND "forloop"] @ forCommand))] in 
