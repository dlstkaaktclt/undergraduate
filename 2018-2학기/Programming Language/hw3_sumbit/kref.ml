...
sig
  ...
  type id = string
  type exp =
    | NUM of int | TRUE | FALSE | UNIT (* constant *)   fin 
    | VAR of id  (* variable *)							fin 
    | ADD of exp * exp									fin
    | SUB of exp * exp									fin
    | MUL of exp * exp									fin
    | DIV of exp * exp									fin
    | EQUAL of exp * exp								fin 
    | LESS of exp * exp 								fin 
    | NOT of exp										fin 
    | SEQ of exp * exp            (* sequence *)		fin 
    | IF of exp * exp * exp       (* if-then-else *)	fin
    | WHILE of exp * exp          (* while loop *)	    fin 
    | LETV of id * exp * exp      (* variable binding *)  fin 
    | LETF of id * id list * exp * exp (* procedure binding *) fin 
    | CALLV of id * exp list      (* call by value *)   
    | CALLR of id * id list       (* call by referenece *)
    | RECORD of (id * exp) list   (* record construction *)  fin
    | FIELD of exp * id           (* access record field *)  fin 
    | ASSIGN of id * exp          (* assgin to variable *)  fin 
    | ASSIGNF of exp * id * exp   (* assign to record field *)  fin
    | READ of id 											fin
    | WRITE of exp											fin 
...
struct
  ...
  let value_int (v : exp) : int =
    match v with
    | Num n -> n
    | _ -> raise (Error "TypeError : not int")

  let value_bool (v : exp) : bool =
    match v with
    | Bool b -> b
    | _ -> raise (Error "TypeError : not bool")

  let value_unit (v : exp) : unit =
    match v with
    | Unit -> ()
    | _ -> raise (Error "TypeError : not unit")

  let value_record (v : exp) : (id * exp) list =
    match v with
    | Record r -> r
    | _ -> raise (Error "TypeError : not record")

  let lookup_env_loc (e : env) (x : id) : loc =
    try
