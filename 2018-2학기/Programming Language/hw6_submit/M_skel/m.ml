...
struct
  ...
  (* val @+ : ('a -> 'b) -> 'a * 'b -> 'a -> 'b *)
  (* operator 오버로딩... 함수와 페어를 받아서 함수를 리턴. *)
  let store (l, m) p = (l, m @+ p) 
  (* m : function. p : pair, 함수와 페어를 받아서 함수를 리턴. 그냥 바인딩이네.. *)
  let load (_, m) l = m l                
  ...
  let op2fn =
    ...
    | EQ -> (fun (v1,v2) -> 
		...
		| (String a,String b) -> (if ((String.compare a b) == 0) then (Bool true) else (Bool false))
		| (Loc a,Loc b) -> (if (a==b) then (Bool true) else (Bool false))
		| _ -> raise (TypeError "equal should get Correct Type!")
		)
		)
  let rec printValue =
    function 
  ...
  let makeRecFun f_id c = 
	let (f,env) = getClosure c in
	match f with
	| Fun (x,e) -> (Closure (RecFun (f_id,x,e),env))
	| _ -> raise (TypeError "none-normal function inputed")
  ...
  let rec eval env mem exp = 
	...
	| LET (decl, e) ->
	  (match decl with
	  | VAL (x_id,x_exp) ->
		let (v1,m') = eval env mem x_exp in
		let (v2,m'') = eval (env @+ (x_id,v1)) m' e in
		(v2,m'')
	  | REC (f_id,a_id,f_exp) -> 
	    let f = RecFun (f_id,a_id,f_exp) in
		let (v,m') = eval (env @+ (f_id,Closure (f,env))) mem e in
		(v,m')
		(* RecLet 정의 무엇???????????
		let (v1, m') = eval env mem f_exp in
		let (v2, m'') = eval (env @+ (f_id,makeRecFun f_id v1)) m' e in
		(v2,m'')
		*)
	  )
    | IF (e1, e2, e3) ->
	...
	| MALLOC e ->
	  let (v, m') = eval env mem e in
	  let (l,raw_m'') = malloc m' in
	  (Loc (l+1),store (raw_m'') (l+1,v))  
	| ASSIGN (e1,e2) ->
