...
class Solution5 {
	...
	public static void main(String[] args) throws Exception {
		...
		int[] disarr = new int[max_h+1]; // 바로 전 블록이 구멍을 고려하지 않아도 되는 블록의 경우의 수를 담는 배열
		int[] conarr = new int[max_h+1]; // 바로 전 블록이 구멍을 고려해야하는 블록으로 쌓은 경우의 수를 담는 배열 (연속으로 쌓는경우)
		int[] temparr = new int[max_h+1]; // 값을 옮길때 사용하는 임시배열
		/*
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			int disarrsize = H+1; // 바로 전 블록이 구멍을 고려하지 않아도 되는 블록의 경우의 수를 담는 배열의 크기
			int conarrsize = H+1; // 바로 전 블록이 구멍을 고려해야하는 블록으로 쌓은 경우의 수를 담는 배열의 크기 (연속으로 쌓는경우)
			int temparrsize = H+1; // 값을 옮길때 사용하는 임시배열의 크기
			...
			for(int i=1;i<n;i++)			// 블록의 개수만큼
			{
				...
				for(int j=1;j<disarrsize;j++)		// 높이의 합이 j인 경우부터 하나씩 체크
				{
					/*
